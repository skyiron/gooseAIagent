...
use mcp_core::{Content, ToolResult, role::Role};
use serde_json::json;
...
async fn submit_tool_result(
    ...
    tracing::info!("Received tool result request: {}", serde_json::to_string_pretty(&raw.0).unwrap());
    ...
    let payload: ToolResultRequest = match serde_json::from_value(raw.0.clone()) {
        ...
        Err(e) => {
            ...
            tracing::error!("Raw request was: {}", serde_json::to_string_pretty(&raw.0).unwrap());
