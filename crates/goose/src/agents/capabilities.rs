...
impl Capabilities {
    ...
    pub async fn add_extension(&mut self, config: ExtensionConfig) -> ExtensionResult<()> {
        ...
        match &config {
            ExtensionConfig::Frontend { name: _, tools, instructions } => {
                ...
                if let Some(instructions) = instructions {
                    self.instructions.insert("frontend".to_string(), instructions.clone());
                }
                Ok(())
            }
            _ => {
    ...
    pub async fn dispatch_tool_call(&self, tool_call: ToolCall) -> ToolResult<Vec<Content>> {
        let result = if tool_call.name == "platform__read_resource" {
            ...
            Err(ToolError::ExecutionError("Frontend tool execution required".to_string()))
        } else {
